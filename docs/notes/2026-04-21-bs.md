...
I'll probably want to tap you for all of them to some extent, but the NIH flavored SBIR (or STTR, I can never remember which is which) is the most relevant. That's the one I will write on a business plan to expand the kind of setup we built for ferret tracking to create a modular system to recording full body mocap and integrated eye tracking for any species (mouse, rat, monkey, human, etc) while producing exactly matched data models. I think the NIH will like that way of bridging animal/human studies. 
